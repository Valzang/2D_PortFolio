...
{
	// 플레이어 추가 및 위치 설정
	cPlayer* PlayerObj = new cPlayer(Vec2(1150.f,600.f));
	...
	MonsterObj = new cMonster_Flying;
	MonsterObj->SetPos(Vec2 { 200.f, 200.f});
	MonsterObj->SetFirstY(MonsterObj->GetPos().y);
	AddObject(MonsterObj, GROUP_TYPE::MONSTER);
	...
	PlatformObj->SetPos(Vec2 { 100.f , Resolution.y / 2.f - 180 }); // 중심좌표..?
	AddObject(PlatformObj, GROUP_TYPE::PLATFORM);


	// 2nd Floor ===================================================================
	PlatformObj = new cPlatform_Rotate;
	PlatformObj->SetPos(Vec2{ Resolution.x -(PlatformObj->GetScale().x - 20.f), Resolution.y / 2.f - 40 }); // 중심좌표..?
	AddObject(PlatformObj, GROUP_TYPE::PLATFORM);// _ROTATE);

	PlatformObj = new cPlatform;
	PlatformObj->SetPos(Vec2{ Resolution.x-(Resolution.x / 2.f - 130.f), Resolution.y / 2.f - 40 }); // 중심좌표..?
	AddObject(PlatformObj, GROUP_TYPE::PLATFORM);
